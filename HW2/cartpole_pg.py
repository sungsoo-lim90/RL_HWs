...
num_actions = env.action_space.n # 2 - left or right
state_space = env.observation_space.shape[0] # (4,) numpy array
...
class Agent:
    def __init__(self, policymodel, valuemodel):
        ...
        self.policymodel.build((1,state_space))
        self.valuemodel.build((1,state_space))
        self.optimizer_policy = tf.keras.optimizers.RMSprop(learning_rate = 0.002, rho = 0.99)
        self.optimizer_value = tf.keras.optimizers.Adam(learning_rate = 0.001)
    ...
    def train(self, env, num_episodes = 500):
        ...
        while episode_number < num_episodes:
            ...
            with tf.GradientTape() as policy_tape, tf.GradientTape() as value_tape:
                action, logits = self.action(obs) #action space and probabilities
                value_estimation = self.value_estimation(obs) #value estimation
                obs, reward, done, _ = env.step(np.argmax(action)) #take action
                ...
                loss_value = self.loss_baseline(value_estimation,reward) #loss for the value model -mean squared error 
    ...
    class Memory():
        ...
        @property
        def discounted_rewards(self, gamma=0.95):
        ...
        @property
        def discounted_rewards_value(self, gamma=0.95):
...
num_episodes = 500 #number of total episodes
batch_size = 10 #batch size - update networks every batch size
...
plt.title('REINFORCE with Baseline for CartPole-v0')
plt.show()
